...
func NewSmallSpaceShipEnemy(x, y int16, health int16) g_m.IEnemyShooter {
	cells := []g_m.ICell{
		//1st
		g_o.NewCell(g_m.CellParams(x, y, "#a60bc6ff")),
		g_o.NewCell(g_m.CellParams(x+1, y, "#a60bc6ff")),
		g_o.NewCell(g_m.CellParams(x+2, y, "#a60bc6ff")),
		//2nd
		g_o.NewCell(g_m.CellParams(x+1, y+1, "#a60bc6ff")),
		//3th
		g_o.NewCell(g_m.CellParams(x+1, y+2, "#a60bc6ff")),
	}
...
func (e *SmallSpaceShipEnemy) Shot() []g_m.IBullet {
	...
	bullet := game_objects.NewEnemyBullet(bottomMidCell.Coords(), "#d48003ff")
